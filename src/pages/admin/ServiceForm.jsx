...
export default function AdminServiceForm() {
  ...
  const { users, handleServiceStatusChange, handleServiceSubmit, handleServiceDelete, toast } = useContext(MessageContext);
  ...
  const handleServiceItemSubmit = async (event) => {
    ...
    const updated = await handleServiceSubmit(prepareServiceDataAtCombined(serviceForm));
    if (updated) {
  ...
  return (
    <>
      <section id="serviceItem" className="flex flex-row flex-wrap justify-between items-center gap-10">
        <h1>{serviceForm.serviceNumber ? <span className="text-content-hover">รายละเอียดนัดหมาย:</span> : "เพิ่มนัดหมายใหม่"} {serviceForm.serviceNumber && serviceForm.serviceNumber?.toUpperCase()}</h1>
            ...
            <table className="table-responsive">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {serviceForm.customer.firstName || serviceForm.customer.lastName
                      ? `คุณ${serviceForm.customer.firstName} ${serviceForm.customer.lastName}`.trim()
                      : <DataNotFound />}
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td>
                    {serviceForm.customer.serviceAddress.addressLine || serviceForm.customer.serviceAddress.subdistrict || serviceForm.customer.serviceAddress.district
                      ? `${serviceForm.customer.serviceAddress.addressLine} ${serviceForm.customer.serviceAddress.subdistrict} ${serviceForm.customer.serviceAddress.district} ${serviceForm.customer.serviceAddress.province} ${serviceForm.customer.serviceAddress.postcode}`.trim()
                      : <DataNotFound />}</td>
                </tr>
              </tbody>
            </table>
        ...
        <form onSubmit={handleServiceItemSubmit}>
          ...
          <div className="input-col grow xl:w-[calc(50%-10px)] self-start">
            ...
            <div className="input-row sm:flex-row max-md:flex-wrap">
              ...
              <div className="input-group max-md:order-3 md:w-[calc(75%-25px-68px)]">
                ...
                <textarea id="url" className="min-h-10 md:min-h-15.5" name="url" rows="2" value={serviceForm.image.url} onChange={handleServiceImageChange} placeholder="https://res.cloudinary.com/slug/image/upload/v8888/filename.png"></textarea>
              </div>
              <div className="input-group max-sm:order-4 sm:w-[calc(100%-20px-68px)] md:w-[calc(25%-15px)] md:shrink-0">
                ...
                <textarea id="cloudinaryId" className="min-h-10 md:min-h-15.5" name="cloudinaryId" rows="2" value={serviceForm.image.cloudinaryId} onChange={handleServiceImageChange} placeholder="กรอก ID ของรูปภาพจาก cloudinary.com"></textarea>
              </div>
            </div>
          </div>
          <div className="input-col grow xl:w-[calc(50%-10px)] self-start">
          ...
          <div className="button-row max-xs:flex-col xs:justify-between">
            <div className="input-group xs:flex-row-reverse xs:w-fit gap-5">
              <button type="submit" className="button w-full xs:w-fit">{serviceNumber ? "บันทึกข้อมูล" : "เพิ่มบริการ"}</button>
            ...
            {serviceNumber &&
              <div className="input-group xs:w-fit">
                ...
                <button type="button" className="button button-soft button-error w-full xs:w-fit" onClick={handleServiceDelete}>ลบบริการนี้</button>
              </div>
            }
          </div>
        </form>
