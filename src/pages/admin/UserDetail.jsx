...
export default function AdminUserDetail() {
  ...
  const { isDev, users, orders } = useContext(MessageContext);
  ...
  const userOrders = userId ? orders.filter((item) => item.customer.userId === Number(userId)) : [];
                      ...
                      ? <>
                          {latestUserOrders.map((order) => (
                            <tr key={order._id}>
                              ...
                              {isDev &&
                                <td className="bg-neutral-50/60"><button onClick={() => handleOrderItem(order.orderId)}>
